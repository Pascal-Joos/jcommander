...
public class ConverterFactoryTest {
  private static final Map<Class, Class<? extends IStringConverter<?>>> MAP =
      Map.of(HostPort.class, HostPortConverter.class);

  private static final IStringConverterFactory CONVERTER_FACTORY =
      new IStringConverterFactory() {
  ...
  /**
   * Test that main parameters can be used with string converters, either with a factory or from the
   * annotation.
   */
  private void mainWithHostPortParameters(
      IStringConverterFactory f, IStringConverterInstanceFactory f2, IHostPorts a) {
    JCommander jc = new JCommander(a);
  ...
  public void mainWithInstanceFactory() {
    mainWithHostPortParameters(
        null,
        new IStringConverterInstanceFactory() {
          @Override
  ...
  public void mainWithSubcommand() throws Exception {
    ...
    jc.addConverterInstanceFactory(
        new IStringConverterInstanceFactory() {
          @Override
          public IStringConverter<?> getConverterInstance(
              Parameter parameter, Class<?> forType, String optionName) {
