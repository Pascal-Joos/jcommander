...
public class VariableArityTest {
  ...
  public static class ModelGenerationConfig {
    ...
    @Parameter(
        ...
        variableArity = true,
        description =
            "File containing a list of instances and their runtimes on various configurations",
        required = false)
    ...
    @Parameter(
        names = {"--seed"},
        description = "Seed used for PRNG [0 means don't use a Seed]")
    public long seed = 0;

    @Parameter(
        names = "-J",
        description = "Parameters to be passed to child process",
        variableArity = true)
  ...
  public void verifyVariableArity() {
    String input =
        "-m foo --seed 1024 -J --compilation_level WHITESPACE_ONLY --language_in=ECMASCRIPT5 -bar baz -faz -c foo -o foo -f foo -J --more-options";
