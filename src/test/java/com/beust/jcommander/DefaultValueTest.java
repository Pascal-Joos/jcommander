...
public class DefaultValueTest {
  ...
  @Test
  public void
      defaultValueForListParametersGetsOverwrittenWithSpecifiedValueOrStaysAsDefaultOtherwise() {
  ...
  private void testSettingMultipleValuesToListTypeParameters(MyOpts opts) {
    JCommander cmd = new JCommander(opts);
    cmd.parse(
        "-a", "anotherValue", "-a", "anotherValue2", "-b", "anotherValue3", "-b", "anotherValue4");
  ...
  public void missingRequiredPrimitiveParameterWithoutDefaultValueMustRaiseParameterException() {
    class MyRequiredOptsWithDefaultValues {
      ...
      public int
          i; // implicit initialization value does not count as a default, so does not satisfy
      // required = true
    }
