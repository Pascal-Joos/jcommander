...
/**
 * Tests the Parameterized parser for substituting the original parser looking go @Parameter but can
 * be replaced to look for other annotations that define run-time parameters.
 *
...
public class ParameterizedParserTest {
  ...
  static final String[] ARGS =
      new String[] {
        ...
        EXPECTED_VERSION,
        "stackLevel",
        Integer.toString(EXPECTED_STACK_LEVEL),
        "logLevel",
        EXPECTED_LOG_LEVEL
      };
  ...
  /**
   * This test ensures that the additional IParameterizedParser does not break current
   * functionality.
   */
  @Test
  public void standardParameterizedParsingTest() {
    ...
    Assert.assertTrue(
        EXPECTED_VERSION.equals(commandOptions.version), "Version is not " + EXPECTED_VERSION);
    Assert.assertTrue(
        EXPECTED_STACK_LEVEL == commandOptions.subCommands.stackLevel,
        "Stack level field is not" + EXPECTED_STACK_LEVEL);
    Assert.assertTrue(
        EXPECTED_LOG_LEVEL.equals(commandOptions.subCommands.loggingLevel),
        "Log level is not " + EXPECTED_LOG_LEVEL);
  }
  ...
  public void ignoreBridgeMethodsTest() {
    ...
    jCommander.parse(
        "--direct-property", "SomeDirectProperty", "--base-property", "SomeBaseProperty");
  ...
  public void testFields(JCommander jcommander, Map<String, Boolean> expectedMap) {
    ...
    for (ParameterDescription paramDesc : paramDescs) {
      Assert.assertTrue(
          expectedMap.containsKey(paramDesc.getNames()),
          "Parameter not in the expected: " + paramDesc.getNames());
      expectedMap.put(paramDesc.getNames(), Boolean.TRUE);
    }

    Assert.assertTrue(
        expectedMap.containsKey(StandardCommandClassExample_01.PARAM_VERSION),
        "Version field not found");
    Assert.assertTrue(
        expectedMap.containsKey(StandardCommandClassExample_02.PARAM_STACK_LEVEL),
        "Stack level field not found");
    Assert.assertTrue(
        expectedMap.containsKey(StandardCommandClassExample_02.PARAM_LOG_LEVEL),
        "Log level field not found");
  }
}
