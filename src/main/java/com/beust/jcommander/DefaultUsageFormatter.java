...
public class DefaultUsageFormatter implements IUsageFormatter {
    ...
    for (Map.Entry<JCommander.ProgramName, JCommander> commands :
      ...
      if (p == null || !p.hidden()) {
        ...
        String commandDescription =
            Optional.ofNullable(getCommandDescription(progName.getName()))
                .map(desc -> s(6) + desc)
                .orElse("");
      ...
      if (!bundleName.isEmpty()) {
        bundle =
            ResourceBundle.getBundle(
                bundleName, Locale.getDefault(), arg.getClass().getClassLoader());
      } else {
  ...
  public void wrapDescription(
      StringBuilder out, int indent, int currentLineIndent, CharSequence description) {
