 ...
 *
 * <p>The object(s) you pass in the constructor are expected to have one or more \@Parameter
...
public class JCommander {
    ...
    public void addValue(Object convertedValue) {
      if (multipleValue != null) {
        ...
        throw new ParameterException(
            "Only one main parameter allowed but found several: "
                + "\""
                + singleValue
                + "\" and \""
                + convertedValue
   ...
   *
   * <p>JCommander supports the {@code @file} syntax, which allows you to put all your options into
   * a file and pass this file as parameter @param expandAtSign whether to expand {@code @file}.
   */
   ...
   * into.
   *
   * @param object The arg object expected to contain {@link Parameter} annotations. If <code>object
   *     </code> is an array or is {@link Iterable}, the child objects will be added instead.
   */
  ...
  /**
   * Sets the {@link ResourceBundle} to use for looking up descriptions. Set this to <code>null
   * </code> to use description text directly.
   */
  // declared final since this is invoked from constructors
  ...
  /**
   * Make sure that all the required parameters have received a value and that all provided
   * parameters have a value compliant to all given rules.
   */
  private void validateOptions() {
    ...
    if (!requiredFields.isEmpty()) {
      ...
      requiredFields
          .values()
          .forEach(
              pd -> missingFields.add("[" + String.join(" | ", pd.getParameter().names()) + "]"));
    ...
    if (mainParameter != null && mainParameter.description != null) {
      ...
      if (mainParameterDescription.getParameter().required()
        ...
        throw new ParameterException(
            "Main parameters are required (\"" + mainParameterDescription.getDescription() + "\")");
      }
    ...
    parametersValidators.forEach(
        parametersValidator -> parametersValidator.validate(nameValuePairs));
  }
      ...
      if (pd.isDynamicParameter()) {
        for (String name : pd.getParameter().names()) {
          if (arg.startsWith(name)
              && !arg.equals(name)
              && arg.contains(pd.getParameter().getAssignment())) {
  ...
  private List<String> readFile(String fileName) {
    ...
    try (BufferedReader bufRead =
        Files.newBufferedReader(Paths.get(fileName), options.atFileCharset)) {
      String line;
  ...
  private void addDescription(Object object) {
    ...
    if (parameters != null) {
      ...
      for (Class<? extends IParametersValidator> parametersValidatorClass :
          parametersValidatorClasses) {
        try {
    ...
    while (i < args.length && !commandParsed) {
      ...
      if (!isDashDash && !"--".equals(a) && isOption(a) && jc == null) {
        ...
        if ("--".equals(arg) && !isDashDash) {
          ...
          for (final Class<? extends IParameterValidator> validator :
              mainParameter.annotation.validateWith()) {
          ...
          if (mainParameter.annotation.converter() != null
            ...
            convertedValue =
                convertValue(
                    ...
                    mainParameter.parameterized.getType(),
                    null,
                    value);
          }
  ...
  private final int determineArity(
      String[] args, int index, ParameterDescription pd, IVariableArity va) {
    List<String> currentArgs = Lists.newArrayList();
    for (int j = index + 1; j < args.length; j++) {
      currentArgs.add(args[j]);
    }
    return va.processVariableArity(
        pd.getParameter().names()[0], currentArgs.toArray(new String[0]));
  }
   ...
   * @return the number of options that were processed.
   */
  private int processVariableArity(
      String[] args, int index, ParameterDescription pd, boolean validate) {
  ...
  private int processFixedArity(
      String[] args, int index, ParameterDescription pd, boolean validate, Class<?> fieldType) {
  ...
  /**
   * Init the main parameter with the given arg. Note that the main parameter can be either a
   * List<String> or a single value.
   */
  private void initMainParameterValue(String arg) {
    if (mainParameter == null) {
      throw new ParameterException(
          "Was passed main parameter '"
              + arg
  ...
  public static class Builder {
     ...
     * into.
     *
     * @param o The arg object expected to contain {@link Parameter} annotations. If <code>object
     *     </code> is an array or is {@link Iterable}, the child objects will be added instead.
     */
     ...
     * Disables expanding {@code @file}.
     *
     * <p>JCommander supports the {@code @file} syntax, which allows you to put all your options
     * into a file and pass this file as parameter @param expandAtSign whether to expand
     * {@code @file}.
     */
    ...
    /**
     * Adds a factory to lookup string converters. The added factory is used prior to previously
     * added factories.
     *
  ...
  /**
   * Adds a factory to lookup string converters. The added factory is used prior to previously added
   * factories.
   *
  ...
  public void addConverterFactory(final IStringConverterFactory converterFactory) {
    addConverterInstanceFactory(
        (parameter, forType, optionName) -> {
          final Class<? extends IStringConverter<?>> converterClass =
              converterFactory.getConverter(forType);
          try {
            if (optionName == null) {
   ...
   */
  public void addConverterInstanceFactory(
      IStringConverterInstanceFactory converterInstanceFactory) {
  ...
  public Object convertValue(
    ...
    if ((type.isAssignableFrom(List.class) || type.isAssignableFrom(Set.class))
        && converter == null) {
      // No list converter: use the single value converter and pass each parsed value to it
      // individually
      final IParameterSplitter splitter = tryInstantiateConverter(null, annotation.splitter());
      converter =
          new DefaultListConverter(
              splitter,
              value1 -> {
                final Type genericType = parameterized.findFieldGenericType();
                return convertValue(
                    parameterized, genericType instanceof Class c ? c : String.class, null, value1);
              });
    }
  ...
  /**
   * The name of the command or the alias in the form it was passed to the command line. <code>null
   * </code> if no command or alias was specified.
   *
